...
void main() {
  ...
  group('sign in', () {
    ...
    testWidgets(
        'WHEN user enters a vand password'
        'AND user taps on the sign-in bualid email tton'
      ...
      await tester.pump(); //to rebuild the state, pumpandsettle for animations

      final signInButton = find.text('Sign In');
      await tester.tap(signInButton);

      verify(mockAuth.signInWithEmailAndPassword(email, password))
          .called(1); //number of times the method needs to be called
